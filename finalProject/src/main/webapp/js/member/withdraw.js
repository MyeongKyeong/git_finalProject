...
$(document).ready(function(){
	...
	$('#withdrawBtn').click(function(){
		...
		else if(! ($('#withdraw_lowFrequency:checked').val() 
				|| $('#withdraw_rejoin:checked').val() 
				|| $('#withdraw_lowContents:checked').val() 
				|| $('#withdraw_protectInfo:checked').val()
				|| $('#withdraw_lowBenefit:checked').val()
				|| $('#withdraw_others:checked').val()))
			$('#reasonDiv').text("1가지 이상의 항목에 꼭 체크하셔야합니다");
		else{
			$('form[name=withdrawForm]').submit();//전송데이터:탈퇴사유 6항목 + 개선사항(주관식) 
